  ...
  return (
    <div className="space-y-6 max-w-4xl animate-in fade-in duration-500">
      <div className="flex items-center justify-between pb-4 border-b border-neutral-850">
        <div>
          <h1 className="text-2xl font-bold text-neutral-100">Formação Acadêmica</h1>
      ...
      {educations.length === 0 ? (
        <div className="flex flex-col items-center justify-center p-12 border border-dashed border-neutral-800 rounded-2xl bg-neutral-900/10 text-center space-y-3">
          ...
          <h3 className="text-lg font-bold text-neutral-300">Nenhuma formação cadastrada</h3>
        ...
        <div className="space-y-4">
          {educations.map((edu) => (
            <Card key={edu.id} className="bg-neutral-900/30 border-neutral-850 hover:border-violet-500/10 transition-all duration-300">
              <CardHeader className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 pb-3">
                <div className="flex items-start gap-4">
                  ...
                  <div className="space-y-1">
                    <CardTitle className="text-lg font-bold text-neutral-100">{edu.course}</CardTitle>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="bg-neutral-900 border-neutral-850 text-neutral-200 max-w-lg overflow-y-auto max-h-[90vh]">
          <DialogHeader>
            <DialogTitle className="text-neutral-100">{editingEducation ? 'Editar Formação' : 'Adicionar Formação'}</DialogTitle>
            <DialogDescription className="text-neutral-500">Insira os dados do seu curso ou graduação.</DialogDescription>
          </DialogHeader>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 pt-2">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="space-y-1.5">
                <Label htmlFor="course" className="text-xs font-semibold text-neutral-400">Curso / Graduação</Label>
                <Input
                  id="course"
                  {...register('course')}
              ...
              {/* Instituição */}
              <div className="space-y-1.5">
                <Label htmlFor="institution" className="text-xs font-semibold text-neutral-400">Instituição de Ensino</Label>
                <Input
                  id="institution"
            ...
            <div className="flex items-center justify-between p-3 rounded-lg bg-neutral-950 border border-neutral-850">
              <div className="space-y-0.5">
                <Label htmlFor="is_current" className="text-xs font-semibold text-neutral-300">Cursando Atualmente</Label>
