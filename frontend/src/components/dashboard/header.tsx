...
export function Header() {
  ...
  return (
    <header className="bg-neutral-900 border-b border-neutral-800 h-16 flex items-center justify-between px-6 z-30 relative">
      {/* Mobile Menu Toggle & Logo */}
      <div className="flex items-center gap-4">
        <button
          onClick={() => setMobileMenuOpen(!mobileMenuOpen)}
          className="md:hidden p-1 text-neutral-400 hover:text-neutral-200 animate-pulse-hover"
        >
      ...
      <div className="flex items-center gap-4 ml-auto">
        <span className="hidden sm:inline text-sm text-neutral-400">
          Olá, <strong className="text-neutral-200 font-semibold">{user?.profile?.name || user?.email}</strong>
        </span>
        ...
        >
          ...
          <Moon className="w-5 h-5 block dark:hidden text-violet-400" />
        </button>

        {/* Logout Button */}
        <button
          onClick={handleLogout}
          className="flex items-center gap-2 text-sm text-red-400 hover:text-red-300 hover:bg-red-950/20 px-3 py-2 rounded-lg transition-colors cursor-pointer"
        >
      ...
      {mobileMenuOpen && (
        <div className="absolute top-16 left-0 right-0 bg-neutral-900 border-b border-neutral-800 p-4 space-y-1 shadow-xl md:hidden flex flex-col z-50 animate-in slide-in-from-top duration-200">
          {menuItems.map((item) => {
          ...
          {user?.profile?.username && (
            <div className="pt-4 border-t border-neutral-800 mt-2">
              <a
                ...
                onClick={() => setMobileMenuOpen(false)}
                className="flex items-center justify-center gap-2 w-full py-2.5 px-4 rounded-lg bg-neutral-800 text-xs font-semibold text-neutral-300"
              >
